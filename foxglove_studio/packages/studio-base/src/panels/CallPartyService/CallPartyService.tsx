...
import { Config } from "@foxglove/studio-base/panels/CallPartyService/types";
...
const useStyles = makeStyles<{ buttonColor?: string }>()((theme, { buttonColor }) => {
  ...
  return {
    ...
    button: {
      ...
      width: "-webkit-fill-available",
      fontSize: "200%",
      backgroundColor: augmentedButtonColor?.light,
      color: 'black',
...
export function CallService({ context }: Props): JSX.Element {
  ...
  return (
    <ThemeProvider isDark={colorScheme === "light"}>
...
function CallServiceContent(
  ...
  return (
    <Stack flex="auto" gap={1} padding={1.5} position="relative" fullHeight >
      ...
      >
        <Tooltip title={config.buttonTooltip}  className={classes.fullHeight}>
