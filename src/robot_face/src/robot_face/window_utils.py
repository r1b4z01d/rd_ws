...
def _parse_hotkey(combo: str) -> list[str]:
    """Split a hotkey string like 'ctrl+shift+h' into tokens."""
    if not combo:
        return []
...
def is_hotkey_pressed(combo: str) -> bool:
    """Check if a multi-key chord like 'ctrl+shift+h' is pressed globally.

    Supports: ctrl, shift, alt, win/super/meta modifiers plus a single final key.
    Falls back to simple key check if parsing fails.
    """

    parts = _parse_hotkey(combo)
    if not parts:
        return False

    # Extract modifiers and the main key (last token) for a simple model.
    main_key = parts[-1]
    modifiers = parts[:-1]
    ...
    if _USER32 is not None:
        ...
        # Main key must be a single character
        if len(main_key) != 1:
            return False
        virtual_key = ord(main_key.upper())
        if not (_USER32.GetAsyncKeyState(virtual_key) & 0x8000):
            return False
    ...
    # X11 path (Linux). We approximate by checking all listed keys are down.
    if _X11 is not None and _X11_DISPLAY:
        keymap = (ctypes.c_ubyte * 32)()
        _X11.XQueryKeymap(_X11_DISPLAY, ctypes.cast(keymap, ctypes.POINTER(ctypes.c_char)))
        ...
        def _pressed(name: str) -> bool:
            if len(name) == 1:
                keycode = _linux_keycode_for_char(name)
            else:
                # modifiers
                sym_map = {
                    "ctrl": "Control_L",
                ...
                if sym is None:
                    return False
                keycode = _linux_keycode_for_char(sym)
            if keycode is None:
                return False
            byte_index = keycode // 8
            bit_index = keycode % 8
            return bool(keymap[byte_index] & (1 << bit_index))

        # All modifiers and main key must be pressed
        if not _pressed(main_key):
            return False
        for mod in modifiers:
            if not _pressed(mod):
                return False
    ...
    # Fallback: simple key only
    return is_global_key_pressed(main_key)
